...
class TableObject {
    ...
    protected function _checkTableExist() {
        $cf = \API\Configurator::$connectionInformationSchema;
        $exist = $cf()->fetch("SELECT * FROM `TABLES` WHERE table_name = ? AND TABLE_SCHEMA = ?", $this->tableName, \API\Configurator::$config["database"]["db_name"]);
    ...
    protected function _loadTableInfo() {
        if (class_exists("Memcache")) {
            $this->columnInfo = \API\Configurator::$memcache->get("TableObjectColumnInfo:" . $this->tableName);
        }
        ...
        if (empty($this->columnInfo)) {
            try {
                $cf = \API\Configurator::$connectionInformationSchema;
                $column_infos = $cf()->query("SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = ? AND table_schema = ?", $this->tableName, \API\Configurator::$config["database"]["db_name"]);
                ...
                if (class_exists("Memcache")) {
                    $column_infos = \API\Configurator::$memcache->set("TableObjectColumnInfo:" . $this->tableName, $this->columnInfo);
                }
